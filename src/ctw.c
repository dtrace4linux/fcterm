...
/*   ESC [9Xm	Bright black foreground                	      	      */
/*   ESC [10Xm	Bright black background                	      	      */
...
#define offset(field) XtOffset(CtwWidget, ctw.field)
	...
	{ XtNlinkForeground, XtCLinkForeground, XtRPixel, sizeof(unsigned long),
	  offset(link_fg), XtRString, "#f0b0b0"},
	{ XtNlinkBackground, XtCLinkBackground, XtRPixel, sizeof(unsigned long),
	  offset(link_bg), XtRString, "#000000"},
...
//printf("len1=%d %d\n", strlen(rp), strlen(keymapping[k].k_value));
			reason.len = strlen(rp);
			XtCallCallbacks((Widget) w, XtNkbdCallback, (caddr_t) &reason);
			RETURN();
			}
...
{	int	flag = TRUE;
	...
	switch (cmd) {
	  case 'c':
	  	/***********************************************/
	  	/*   Linux cursor sequences.		       */
	  	/***********************************************/
		if (args[0])
			ctw->ctw.cursor_style = args[0];
		if (args[1])
			ctw->ctw.c_cursor_fg = args[1];
		if (args[2])
			ctw->ctw.c_cursor_bg = args[2];
	  	return TRUE;
...
static int done_read = FALSE;
	...
	for (vp = lp->l_text ; vp + len < vpend; vp++) {
		...
		if (vp1 + 4 >= vpend)
			continue;
		if (vp1->vb_byte == 's' && vp1[1].vb_byte == ':') {
			vp1++;
			len++;
			}
		if (vp1->vb_byte != ':')
		...
		if (vp1->vb_byte != '/')
			continue;
		vp1++, len++;
		for (; vp1 < vpend; vp1++, len++) {
			if (strchr(", ;<>(){}!", vp1->vb_byte) != NULL)
				break;
			}

		for (j = 0; j < len; j++)
			map[vp - lp->l_text + j] |= MAP_LINK;
		vp += len;
		}
	/***********************************************/
	...
	for (c = col; c < end_col; ) {
		...
		while (bp < end_buf) {
			if (vp->vb_fcolor != attr.vb_fcolor ||
			    ...
			    vp->vb_attr2 != attr.vb_attr2)
				break;
			if (vp->vb_attr2 & VB2_FG_24 &&
			    memcmp(vp->vb_fg, attr.vb_fg, 3) != 0)
			    	break;
			if (vp->vb_attr2 & VB2_BG_24 &&
			    memcmp(vp->vb_bg, attr.vb_bg, 3) != 0)
			    	break;
		...
		if (attr.vb_attr & VB_SELECT) {
			...
			fg = attr.vb_attr2 & VB2_FG_24 
				? (Pixel) ((int) attr.vb_fg[0] << 16) | ((int) attr.vb_fg[1] << 8) | ((int) attr.vb_fg[2])
		...
		if (sline[c] & MAP_LINK) {
			fg = ctw->ctw.link_fg;
			bg = ctw->ctw.link_bg;
			attr.vb_attr |= VB_UNDERLINE | VB_BOLD;
			}
		else if (sline[c]) {
...
/*			arg_no = 1;
	  	...
	  	for (i = 0; i < arg_no; i++) {
			switch (args[i]) {
			  ...
			  case 38: 
			  	/***********************************************/
			  	/*   ESC [ 38 ; 2 ; rr ; gg ; bb ; ... color   */
			  	/*   ESC [ 38 ; 5 ; nn m - palette color       */
			  	/***********************************************/
				switch (args[i+1]) {
				  case 2:
				  	ctw->ctw.attr.vb_fg[0] = args[i+2];
				  	ctw->ctw.attr.vb_fg[1] = args[i+3];
				  	ctw->ctw.attr.vb_fg[2] = args[i+4];
					ctw->ctw.attr.vb_attr2 |= VB2_FG_24;
					i += 5;
					break;
			  ...
			  case 48: 
			  	/***********************************************/
			  	/*   ESC [ 48 ; 2 ; rr ; gg ; bb ; ... color   */
			  	/*   ESC [ 48 ; 5 ; nn m - palette color       */
			  	/***********************************************/
				switch (args[i+1]) {
				  case 2:
				  	ctw->ctw.attr.vb_bg[0] = args[i+2];
				  	ctw->ctw.attr.vb_bg[1] = args[i+3];
				  	ctw->ctw.attr.vb_bg[2] = args[i+4];
				  	ctw->ctw.attr.vb_attr2 |= VB2_BG_24;
					i += 5;
					break;
			  ...
			  case DRAW_DUMP_STATUS: { /* 1935 */
				...
				/***********************************************/
			  	snprintf(buf, sizeof buf, "%s/fcterm.%d.status.tmp", log_dir, getpid());
			  	snprintf(buf2, sizeof buf2, "%s/fcterm.%d.status", log_dir, getpid());
				remove(buf);
				FILE *fp = fopen(buf, "w");
				if (fp == NULL) {
